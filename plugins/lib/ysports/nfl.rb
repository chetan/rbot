...
class NFL < Base
    ...
    def self.find_team_page(str)
        ...
        begin
            html = YSports.fetchurl('http://sports.yahoo.com/nfl/teams/' + str)
        rescue => ex
            puts ex
            return
        end
        ...
        if search then
            ...
            return [ team[0], YSports.fetchurl('http://sports.yahoo.com/nfl/teams/' + team[0]) ]
    ...
    def self.parse_schedule(html)
        ...
        for i in 1..17 do # only want the first 17 entries
            ...
            date = YSports.strip_tags(sched.shift)
            team = YSports.strip_tags(sched.shift)
            time_or_score = YSports.strip_tags(sched.shift)
